      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-semibold text-lg">
            Your documents
            {documents.length > 0 && (
              <span className="text-muted-foreground font-normal text-sm ml-2">({documents.length})</span>
            )}
          </h2>
          <Button variant="ghost" size="sm" onClick={fetchDocuments} className="gap-1.5 text-xs">
        ...
        {loading ? (
          ...
          <div className="text-center py-16 rounded-xl border-2 border-dashed">
            ...
            <p className="text-muted-foreground text-sm">Upload your first PDF to get started</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {documents.map((doc) => (
