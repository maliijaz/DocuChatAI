...
export default async function BillingPage({
  ...
  const plans = [
    { key: "FREE" as Plan, ...PLAN_DETAILS.FREE },
    { key: "PRO" as Plan, ...PLAN_DETAILS.PRO },
    { key: "ENTERPRISE" as Plan, ...PLAN_DETAILS.ENTERPRISE },
  ];

  return (
    <div className="space-y-8 max-w-4xl">
      <DashboardHeader title="Billing & Plans" description="Manage your subscription and upgrade your plan." />
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {plans.map((p) => (
            <Card
              key={p.key}
              className={`relative ${p.key === plan ? "border-primary ring-1 ring-primary" : ""}`}
            >
              {p.key === plan && (
