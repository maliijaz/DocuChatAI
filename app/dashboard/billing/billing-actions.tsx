...
export function BillingActions({ plan, hasSubscription }: BillingActionsProps) {
  ...
  if (hasSubscription && !plan) {
    return (
      <Button variant="outline" size="sm" onClick={handlePortal} disabled={loading} className="gap-2">
  ...
  if (plan) {
    return (
      <Button className="w-full gap-2" size="sm" onClick={handleUpgrade} disabled={loading} variant="gradient">
